...
import {
  ...
  templateMatchingImageRatio,
  templateMatchingImageSize,
  templateMatchingGreenThresholdPercentage,
...
const initTemplateAsync = (cv) => async (file) => {
  ...
  let templateMatchingImage = imageResize(cv)(
    imgCvTemplate,
    templateMatchingImageSize * templateMatchingImageRatio
  ).image;
  ...
  let imgCvTemplateResizedMatch = imageResize(cv)(
    imgCvTemplate,
    templateMaximumSize
  ).image;
  const featureMatchingDetectAndComputeSerializable =
    detectAndComputeSerializable(cv)(imgCvTemplateResizedMatch);
...
const loadOpenCVAsync = async (
  openCVScript = `https://docs.opencv.org/4.9.0/opencv.js`
) => {
...
const texts = {
  "sc-modal__video-title":
    "Positionner 5 secondes votre document dans le cadre",
  ...
  "sc-modal__confirm-title":
    "Est-ce que tous les champs sont parfaitement lisibles ?",
...
function validateProperties(internal_properties) {
  ...
  if (
    final_properties.outputImageMimeType !== "image/jpeg" &&
    final_properties.outputImageMimeType !== "image/png"
  ) {
    throw new Error("outputImageMimeType must be image/jpeg or image/png");
  }
  if (
    final_properties.outputImageQuality < 0 ||
    final_properties.outputImageQuality > 1
  ) {
...
export const loadVideoAsync =
  (name) =>
  (cv = null) =>
  async (file, onCaptureCallback = null, internal_properties = properties) => {
    if (!openCVPromise) {
    ...
    const {
      ...
      templateMatchingImage,
    } = await initTemplateAsync(cv)(file);

    return await captureAsync(cv)(
      name,
      featureMatchingDetectAndComputeSerializable,
      templateMatchingImage,
      onCaptureCallback,
...
const displayError = (
  ...
  iButtonRestart.className =
    "sc-modal__error-button sc-modal__error-button--restart";
  ...
  (
    ...
    return new Promise((resolve, error) => {
      ...
      const text = document.createTextNode(
        translations["sc-modal__video-title"]
      );
      iH1.appendChild(text);
      iH1.id = cuid();
      ...
      if (enableDefaultCss) {
        iButton.style =
          "padding:0.5em;font-size:1em;margin:1em;position:absolute;background-color:#a8a8a842;";
      }
      iButton.onclick = () => {
        stopStreaming();
        constraints.video.facingMode.ideal =
          constraints.video.facingMode.ideal === "face"
            ? "environment"
            : "face";
      ...
      if (enableDefaultCss) {
        iButtonQuit.style =
          "padding:0.3em;font-size:1em;margin:1em;position:absolute;top:0;right:0;";
      }
      iButtonQuit.onclick = quit;
      ...
      const restartAsync = async () => {
        document.getElementsByTagName("body")[0].removeChild(iDiv);
        const loadVideo = await captureAsync(cv)(
          name,
          featureMatchingDetectAndComputeSerializable,
          templateMatchingImage,
          onCaptureCallback,
      ...
      async function processVideo(videoCapture, src) {
        ...
        if (frameCounter % 3 === 0) {
          const applyTemplateMatchingResult = await applyTemplateMatching(cv)(
            templateMatchingImage,
            imageDestination
          );
          processVideoCache.imageCv = applyTemplateMatchingResult.imageCv;
          processVideoCache.targetPoints =
            applyTemplateMatchingResult.targetPoints;
          processVideoCache.currentPoints =
            applyTemplateMatchingResult.currentPoints;
          processVideoCache.autoAdjustBrightnessRatio =
            applyTemplateMatchingResult.autoAdjustBrightnessRatio;
        ...
        const point1TargetRectangle = new cv.Point(
          Math.round(targetPoints.x1 * cols),
          Math.round(targetPoints.y1 * rows)
        );
        const point2TargetRectangle = new cv.Point(
          Math.round(targetPoints.x2 * cols),
          Math.round(targetPoints.y2 * rows)
        );

        let colorRectangle;
        let sizeRectangle;
        if (currentPoints) {
          const templateMatchingPercentage =
            Math.abs(currentPoints.x1 - targetPoints.x1) * 100;
          if (
            templateMatchingPercentage <
            templateMatchingGreenThresholdPercentage
          ) {
        ...
        cv.rectangle(
          imageDestination,
          point1TargetRectangle,
          point2TargetRectangle,
          colorRectangle,
          sizeRectangle,
          cv.LINE_8,
          0
        ...
        if (currentPoints != null) {
          ...
          let colorGreen = new cv.Scalar(
            50,
            green,
            255 - Math.round(255 / subRatio),
            255
          );
          cv.putText(
            imageDestination,
            (waitNumber - counterTime).toString(),
            new cv.Point(
              Math.round(cols / 2 - size.width / 2),
              Math.round(rows / 2 - size.height / 2)
            ),
            font,
            fontScale,
            colorGreen,
        ...
        if (autoAdjustBrightnessRatio > thresholdTooDark) {
          ...
          cv.putText(
            imageDestination,
            translations["sc-modal__video-message-too-dark"],
            new cv.Point(
              Math.round(size.width * 0.12),
        ...
        if (autoAdjustBrightnessRatio < thresholdTooWhite) {
          ...
          cv.putText(
            imageDestination,
            translations["sc-modal__video-message-too-white"],
            new cv.Point(
              Math.round(size.width * 0.12),
              Math.round(rows * 0.12)
            ),
        ...
        if (counterTime > waitNumber) {
          ...
          iHLoading.id = cuid();
          const text = document.createTextNode(
            translations["sc-modal__confirm-loading"]
          );
          ...
          await delay(50);
          const zoneResult = await zoneAsync(cv)(
            finalShot,
            featureMatchingDetectAndComputeSerializable,
            60,
            targetPoints
          ...
          iH1.id = cuid();
          const textConfirmTitle = document.createTextNode(
            translations["sc-modal__confirm-title"]
          );
          ...
          iButtonNo.className =
            "sc-modal__confirm-button sc-modal__confirm-button--ko";
          ...
          iButtonYes.className =
            "sc-modal__confirm-button sc-modal__confirm-button--ok";
          iButtonYes.textContent = translations["sc-modal__confirm-button--ok"];
          iButtonYes.onclick = async () => {
            const blob = await toBlobAsync(cv)(
              zoneResult.finalImage,
              outputImageMimeType,
              outputImageQuality
            );
            zoneResult.finalImage.delete();
      ...
      const start = async (enableManualErrorHandled) => {
        try {
          for await (const val of startStreaming()) {
          }
        } catch (err) {
          if (enableManualErrorHandled !== true) {
